...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        if model_name_or_path.endswith('.nemo'):
          asr_model = ASRModel.restore_from(model_name_or_path)
        else:
          asr_model = ASRModel.from_pretrained(model_name_or_path)
        ...
        self.decoding = asr_model.decoding
        self.preprocessor = self.extract_preprocessor(asr_model, "cuda")
        self.encoder = TrtInfer(parameters['engine_file']['string_value'])
 
    def extract_preprocessor(self, model, device):
    ...
    def execute(self, requests):
        ...
        with torch.inference_mode():
            ...
            # preprocessor
            processed_example, processed_example_length = \
                              self.preprocessor(input_signal=waveform_batch, 
                                                length=lengths)
            
            # encoder
            log_probs, encoded_len = self.encoder.infer_encoder((processed_example, processed_example_length))
            
            transcribed_texts, _ = self.decoding.ctc_decoder_predictions_tensor(
